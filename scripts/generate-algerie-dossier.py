#!/usr/bin/env python3
"""
Generate Algerian Market Development Dossier in Arabic
- Creates a comprehensive 14-slide PPTX presentation using pptxgenjs (via Node.js)
- Covers market overview, agriculture, water usage, climate, technology, case studies, etc.
- Outputs to public/documents/countries/algerie.pdf
"""
...
def create_dossier():
    """Create the Algerian market development dossier."""

    project_root = Path(__file__).parent.parent
    scripts_dir = project_root / "scripts"
    countries_dir = project_root / "public" / "documents" / "countries"

    # Ensure output directory exists
    countries_dir.mkdir(parents=True, exist_ok=True)

    # Create the Node.js script to generate the PPTX
    nodejs_script = scripts_dir / "generate-algerie-dossier.js"
...
prs.layout = "LAYOUT1";
prs.defineLayout({ name: "16:9", width: 13.333, height: 7.5 });
...
// Arabic text helper - right-to-left text
...
// Save as PPTX
const pptxPath = process.argv[2] || "algerie-dossier.pptx";
prs.save({ path: pptxPath });

console.log(`Presentation saved to ${pptxPath}`);
'''

    with open(nodejs_script, 'w', encoding='utf-8') as f:
        f.write(nodejs_code)

    print(f"Created Node.js script at {nodejs_script}")

    # Run the Node.js script to generate PPTX
    pptx_path = countries_dir / "algerie.pptx"

    print(f"Generating PPTX presentation...")
    result = subprocess.run(
        [sys.executable, "-c", f'''
import subprocess
import sys
output = subprocess.run([
    "node", r"{str(nodejs_script)}", r"{str(pptx_path)}"
...
'''],
    ...
    if result.returncode != 0:
        # Try direct node call
        result = subprocess.run(
            ["node", str(nodejs_script), str(pptx_path)],
            cwd=str(project_root),
            capture_output=True,
            text=True
        )
    ...
    # Now convert PPTX to PDF using libreoffice
    pdf_path = countries_dir / "algerie.pdf"

    print(f"Converting PPTX to PDF...")
    convert_result = subprocess.run(
        ["libreoffice", "--headless", "--convert-to", "pdf", "--outdir", str(countries_dir), str(pptx_path)],
        capture_output=True,
        text=True
    )

    if convert_result.returncode != 0:
        print(f"Note: LibreOffice conversion may have different behavior")
        print(f"Conversion output: {convert_result.stdout}")
        print(f"Conversion stderr: {convert_result.stderr}")
        # PDF might still have been created, let's check

    # Verify outputs
    if pptx_path.exists():
        pptx_size = pptx_path.stat().st_size
        print(f"✓ PPTX created: {pptx_path}")
        print(f"  Size: {pptx_size:,} bytes")
    else:
        print(f"✗ PPTX not created")
        return False
    ...
    if pdf_path.exists():
        ...
        print(f"! PDF not created via libreoffice, checking for alternatives...")
        # Try with different command or library
        return True  # PPTX is created successfully

if __name__ == "__main__":
    success = create_dossier()
